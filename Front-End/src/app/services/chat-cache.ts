...
export class ChatCacheService {
	...
	private openDB(): Promise<IDBDatabase> {
		if (this.isServer() || typeof indexedDB === "undefined")
	...
	private idbGet(roomId: string): Promise<ChatCacheEntry | undefined> {
		...
		return this.openDB().then(
			(db) =>
				new Promise<ChatCacheEntry | undefined>((resolve, reject) => {
					const tx = db.transaction(this.STORE, "readonly");
					const store = tx.objectStore(this.STORE);
					const req = store.get(roomId);
					req.onsuccess = () =>
						resolve(req.result as ChatCacheEntry | undefined);
					req.onerror = () => reject(req.error);
				}),
		);
	}
	...
	upsertMessages(
		roomId: string,
		newMessagesAsc: HttpChatMessage[],
		markFullyLoaded = false,
	): void {
		const now = Date.now();
		const current = this.memory.get(roomId) ?? {
			roomId,
			messagesAsc: [],
			fullyLoaded: false,
			updatedAt: now,
		...
		const seen = new Set(
			current.messagesAsc.map((m) => m.id).filter(Boolean) as string[],
		);
