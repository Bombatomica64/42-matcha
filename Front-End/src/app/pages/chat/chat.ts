...
export class Chat implements OnInit, OnDestroy {
	...
	private currentUserIdSig = signal<string | null>(null);
	...
	ngOnInit(): void {
		// derive current user id once from token
		this.currentUserIdSig.set(this.tokenStore.userId);
	...
	private normalizeResponse(resp: unknown): HttpChatMessage[] {
		...
		if (
			...
			return (resp as { data: HttpChatMessage[] }).data;
		return [];
	}
	...
	ngAfterContentInit(): void {
		...
		this.route.paramMap.subscribe((pm) => {
			...
			if (newId && newId !== this.chatRoomIdSig())
				this.chatRoomIdSig.set(newId);
		});
	}

	private scrollToBottomSoon(): void {
		// Defer to next frame so scroller has items and ViewChild resolved
