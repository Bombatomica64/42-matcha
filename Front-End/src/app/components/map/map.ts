...
export class Map implements AfterViewInit, OnDestroy, OnChanges {
  ...
  async ngOnChanges(changes: SimpleChanges) {
    ...
    if ((changes['lat'] || changes['lng']) && this.map) {
      ...
      console.log('Map input changed:', { previous: { lat: prevLat, lng: prevLng }, current: { lat: curLat, lng: curLng } });
      const lat = this.lat ?? 45.4642;
      const lng = this.lng ?? 9.1900;
      try {
  ...
  ngOnDestroy(): void {
    ...
    try {
      if (this.mapContainer?.nativeElement && (this.mapContainer.nativeElement as any)._leaflet_id) {
  ...
  private async initMap(): Promise<void> {
    ...
    this.marker.on('dragend', () => {
      ...
      // log and emit asynchronously to avoid sync change-detection issues in parent
      setTimeout(() => {
        console.log('Marker dragend -> new location', { lat: p.lat, lng: p.lng });
