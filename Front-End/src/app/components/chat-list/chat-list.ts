...
export class ChatList implements OnInit {
	...
	ngOnInit(): void {
		...
		this.socket.on("newMessage", (data: unknown) => {
			...
			if (!roomId) return;
			const activeId = this.router.url.includes("/home/chat/")
				? this.router.url.split("/home/chat/")[1]
				: null;
			if (activeId?.startsWith(roomId)) return;
			this.unreadMap.update((curr) => ({
				...curr,
				[roomId]: (curr[roomId] || 0) + 1,
			}));
	...
	private async fetchRooms() {
		try {
			...
			const rooms: ChatRoom[] = Array.isArray(resp)
				? resp
				: Array.isArray((resp as { data?: ChatRoom[] })?.data)
					? ((resp as { data?: ChatRoom[] }).data as ChatRoom[])
					: [];
			...
			type SmallUserShape = {
				...
				main_photo?: PhotoShape;
			};
			const enriched: ChatListItem[] = rooms.map((r) => {
				const su1 = (r as unknown as { small_user1: SmallUserShape })
					.small_user1;
				const su2 = (r as unknown as { small_user2: SmallUserShape })
					.small_user2;
						...
						? [
								{
									...
									image_url: photo.image_url?.startsWith("http")
										? photo.image_url
										: `${this.environment.apiUrl}${photo.image_url}`,
								...
								? [
										{
											...
											image_url: altPhoto.image_url?.startsWith("http")
												? altPhoto.image_url
												: `${this.environment.apiUrl}${altPhoto.image_url}`,
	...
	otherName(item: ChatListItem): string {
		const u = item.otherUser as unknown as
	...
	hasPhoto(item: ChatListItem): boolean {
		const u = item.otherUser as unknown as
			| { photos?: Array<{ image_url?: string; is_main?: boolean }> }
			| undefined;
	...
	mainPhoto(item: ChatListItem): string | undefined {
		...
		if (u?.photos?.length) {
			...
			return main.image_url.startsWith("http")
				? main.image_url
				: `${this.environment.apiUrl}${main.image_url}`;
		}
		return undefined;
	}
	...
	markRead(roomId: string) {
		this.unreadMap.update((curr) => {
			if (!curr[roomId]) return curr;
			const copy = { ...curr };
			delete copy[roomId];
			return copy;
		});
		this.itemsSig.update((list) =>
			list.map((it) => (it.room.id === roomId ? { ...it, unread: 0 } : it)),
		);
	}
}
