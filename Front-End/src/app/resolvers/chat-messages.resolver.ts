...
export const chatMessagesResolver: ResolveFn<ChatPrefetchData | null> = async (route) => {
  ...
  try {
    ...
    if (isPlatformServer(platformId)) {
      ...
      const data = await firstValueFrom(
        http.request(null, `/chat/${roomId}/messages${query}` as HttpEndpoint, 'GET' as HttpMethod)
      );
      const arr: ChatMessage[] = Array.isArray(data) ? data as ChatMessage[] : [];
