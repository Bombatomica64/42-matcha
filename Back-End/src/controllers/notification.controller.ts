...
import { emitAllNotificationsRead, emitNotificationRead } from "../sockets/notifications.socket";
...
export class NotificationController {
	...
	listUnread = async (req: Request, res: Response) => {
		...
		try {
			...
			res
				.status(500)
				.json({ error: "SERVER_ERROR", message: "Failed to list unread notifications" });
		}
	};
	...
	markAllRead = async (_req: Request, res: Response) => {
		...
		try {
			...
			// Emit real-time notification for all read
			...
			res
				.status(500)
				.json({ error: "SERVER_ERROR", message: "Failed to mark all notifications read" });
		}
	};
}
