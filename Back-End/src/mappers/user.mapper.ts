...
export function apiUserToDbUser(apiData: Partial<ApiUser>): Partial<DbUser> {
  return {
    ...
    sexual_orientation: apiData.sexual_orientation === 'other' 
      ? 'bisexual' 
      : apiData.sexual_orientation,
    location: apiData.location && apiData.location.longitude !== undefined && apiData.location.latitude !== undefined ? {
      type: 'Point',
 ...
 */
export function validateUserData(userData: Partial<ApiUser>): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
