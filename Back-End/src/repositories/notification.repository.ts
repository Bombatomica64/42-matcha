...
export class NotificationRepository {
	...
	async getPage(
      ...
      LIMIT $2 OFFSET $3`;
		...
		return {
			rows: listResult.rows as NotificationRow[],
	...
	async create(notification: {
		...
		const result = await this.pool.query(
			`INSERT INTO notifications (user_id, actor_id, type, metadata, status, created_at)
       VALUES ($1, $2, $3, $4, 'pending', NOW())
       RETURNING id, user_id, actor_id, type, read_at, delivered_at, status, metadata, created_at`,
			[notification.user_id, notification.actor_id, notification.type, notification.metadata],
		);
	...
	async markDelivered(notificationId: string): Promise<void> {
		await this.pool.query(
			`UPDATE notifications SET status = 'sent', delivered_at = NOW()
