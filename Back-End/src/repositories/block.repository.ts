...
export class BlockRepository extends BaseRepository<UserBlock> {
   ...
   */
  async isBlockedBetweenUsers(user1Id: string, user2Id: string): Promise<{
   ...
   */
  async getBlockedUsers(blockerId: string, limit = 50, offset = 0): Promise<BlockedUserWithDetails[]> {
    const query = `
      SELECT 
        ub.*,
