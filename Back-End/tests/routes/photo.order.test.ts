import fs from "node:fs";
import path from "node:path";
import { afterAll, beforeAll, beforeEach, describe, expect, it } from "@jest/globals";
import type { Express } from "express";
import request from "supertest";
import { server } from "../../src/server";
import { createTestApp } from "../helpers/app.helper";
import { createUserAndAccessToken } from "../helpers/auth.helper";
import { clearDatabase, closeTestPool, seedTestData } from "../helpers/database.helper";

describe("Photo Reorder Route", () => {
	let app: Express;
	let authToken: string;
	let testImagePath: string;

	beforeAll(async () => {
		app = await createTestApp();

		// Prepare a tiny JPEG fixture
		testImagePath = path.join(__dirname, "../fixtures/test-image-reorder.jpg");
		const dir = path.dirname(testImagePath);
		if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
		const minimalJpeg = Buffer.from([
			,
		]);
		fs.writeFileSync(testImagePath, minimalJpeg);
	});

	afterAll(async () => {
		await closeTestPool();
		server.close();
		if (fs.existsSync(testImagePath)) fs.unlinkSync(testImagePath);
	});

	beforeEach(async () => {
		await clearDatabase();
		await seedTestData();

		const { token } = await createUserAndAccessToken({
			email: "reorder@example.com",
			username: "reorderuser",
			firstName: "Reorder",
			lastName: "User",
			password: "SecurePassword123!",
			birthDate: "1990-01-01",
			gender: "male",
		});
		authToken = token;
	});

	it("should reorder photos successfully", async () => {
		// Upload 3 photos
		const ids: string[] = [];
		for (let i = 0; i < 3; i++) {
			const res = await request(app)
				.post("/photos")
				.set("Authorization", `Bearer ${authToken}`)
				.attach("photo", testImagePath)
				.expect(201);
			// API returns { message, photo }
			ids.push(res.body.photo.id);
		}

		const reversed = [...ids].reverse();
		await request(app)
			.put("/photos/order")
			.set("Authorization", `Bearer ${authToken}`)
			.send({ photoIds: reversed })
			.expect(204);

		// Verify order via GET /photos
		const list = await request(app)
			.get("/photos")
			.set("Authorization", `Bearer ${authToken}`)
			.expect(200);
		const listedIds: string[] = (list.body.photos ?? []).map((p: { id: string }) => p.id);
		expect(listedIds).toEqual(reversed);
	});

	it("should return 400 for missing body", async () => {
		const res = await request(app)
			.put("/photos/order")
			.set("Authorization", `Bearer ${authToken}`)
			.send({})
			.expect(400);
		expect(res.body).toHaveProperty("error");
	});

	it("should return 401 without auth", async () => {
		await request(app).put("/photos/order").send({ photoIds: [] }).expect(401);
	});

	it("should return 400 when including non-owned photo id", async () => {
		// Upload one photo for current user
		const mine = await request(app)
			.post("/photos")
			.set("Authorization", `Bearer ${authToken}`)
			.attach("photo", testImagePath)
			.expect(201);

		// Create another user and upload a photo
		const otherUser = await createUserAndAccessToken({
			email: "other@example.com",
			username: "otheruser",
			firstName: "Other",
			lastName: "User",
			password: "SecurePassword123!",
			birthDate: "1991-01-01",
			gender: "female",
		});
		const otherPhoto = await request(app)
			.post("/photos")
			.set("Authorization", `Bearer ${otherUser.token}`)
			.attach("photo", testImagePath)
			.expect(201);

		await request(app)
			.put("/photos/order")
			.set("Authorization", `Bearer ${authToken}`)
			.send({ photoIds: [otherPhoto.body.photo.id, mine.body.photo.id] })
			.expect(400);
	});
});
